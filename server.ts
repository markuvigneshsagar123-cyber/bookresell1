...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    const stmt = db.prepare('INSERT INTO users (id, displayName, email, password) VALUES (?, ?, ?, ?)');
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id, email: user.email, name: user.displayName }, JWT_SECRET);
...
app.post('/api/books', upload.single('image'), (req, res) => {
  const { title, author, price, category, condition, description, sellerId, sellerName, quantity } = req.body;
